...
{
return ((-3 + 2*sqr(q[m]))*(q[m]*Vq[m]*(-3 + 2*sqr(q[m])) + V*(-3 + 10*sqr(q[m]))))/6.;
}
...
{
return (4*V*q[j]*q[l]*q[m]*q[n] + (q[l]*q[m]*q[n]*Vq[j] + q[j]*(q[m]*q[n]*Vq[l] + q[l]*q[n]*Vq[m] + q[l]*q[m]*Vq[n]))*(-1 + 2*sqr(q[m])))/(2.*sqrt(2));
}
