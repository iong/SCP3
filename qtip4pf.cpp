...
double monomer(const double* RESTRICT w, double* RESTRICT grad)
{
    double d1[3], d2[3];
...
void qtip4pf::allocate(size_t nw)
{
    ...
    // see operator() below
    m_mem = new double[3*nw + 3*nw + 3*nw];
...
double qtip4pf::operator()(size_t nw, const double* RESTRICT crd)
{
    assert(crd || nw == 0);
...
const double* qtip4pf::dip(size_t nw, const double* crd)
{
    assert(nw > 0 && nw == m_nw);

    // setup pointers (keep in sync with operators () above)
    double* msite = m_mem;
    double* dM = msite + 3*nw;
    double* dip = dM + 3*nw;
    ...
    for (size_t n = 0; n < m_nw; ++n) {
        ...
        dip[n3 + 2] += qM*msite[n3 + 2] + qH*crd[n9 + 5] + qH*crd[n9 + 8];
    }
